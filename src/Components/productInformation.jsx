...
function ProductInformation({productID}) {
    ...
    return (
        <div className={'flex flex-col justify-center p-3 gap-16 lg:flex-row lg:items-center '}>

            <div className={'flex flex-col gap-6 lg:w-2/4 overflow-hidden md:max-w-md '}>
                <img src={activeImg} className={'w-full h-full aspect-square object-cover'} alt={'imagen del producto'}/>
                <div className={'flex flex-row justify-between h-16 md:h-24'}>

                    {product.imagePath.map((images) => (
                        <img
                            src={images}
                            className={'h-16 w-16 md:h-24 md:w-24  rounded-md object-cover cursor-pointer'}
            ...
            <div className={'flex flex-col gap-4 lg:w-2/4'}>
                <div>
                    <span className={'text-blue-300 font-semibold'}>Codigo: {productID}</span>
                ...
                <div className={'flex flex-row gap-5 items-center'}>
                    ...
                    <a className={'bg-blue-800 text-white font-semibold py-3 px-6 block flex-grow' +
                        ' text-center hover:bg-blue-900'}>Add to cart</a>
                </div>
            </div>
        </div>
    )
